...
# New constraints
MIN_BASELINE_ANGLE_DEG = 30.0  # Minimum angle between baselines
MAX_BASELINE_LENGTH_M = 30000.0  # Maximum sensor-IoO distance
MIN_ALTITUDE_M = 0.0
MAX_ALTITUDE_M = 30000.0
MIN_VELOCITY = 50.0  # m/s
MAX_VELOCITY = 500.0  # m/s
MIN_VERTICAL_VELOCITY = -50.0  # m/s (descending)
MAX_VERTICAL_VELOCITY = 50.0   # m/s (ascending)
...
def calculate_doppler(sensor: Position, ioo: Position, target: Position, velocity: Velocity) -> float:
    """Calculate Doppler shift including vertical component."""
    sensor_ecef = lla_to_ecef(sensor.lat, sensor.lon, sensor.alt)
    ioo_ecef = lla_to_ecef(ioo.lat, ioo.lon, ioo.alt)
    target_ecef = lla_to_ecef(target.lat, target.lon, target.alt)
    ...
    # Velocity vector (approximate ECEF from ENU)
...
def generate_sensor_ioo_configuration(center_lat: float, center_lon: float) -> Tuple[List[Position], List[Position], List[Tuple[int, int]]]:
    ...
    for attempt in range(max_attempts):
        ...
        # Option 1: Two sensors share IoO 0, one uses IoO 1
        configurations = [
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Generate 3-detection test cases')
    parser.add_argument('--num-cases', type=int, default=5, help='Number of test cases')
    parser.add_argument('--output-dir', default='test_3detections', help='Output directory')
    args = parser.parse_args()
    
    # Create output directory
    os.makedirs(args.output_dir, exist_ok=True)
    
    print(f"Generating {args.num_cases} test cases with 3 detections...\n")
    ...
    for i in range(1, args.num_cases + 1):
        try:
            ...
            print(f"    Sensor-IoO pairs: {case_summary['sensor_ioo_pairs']}")
    ...
    print(f"\n{'='*60}")
    print(f"Summary: Generated {len(all_cases)} test cases")
